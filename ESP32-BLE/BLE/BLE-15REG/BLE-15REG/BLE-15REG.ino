...
void enviarRegistros() {
    ...
    String buffer[30];  // Arreglo para acumular 15 líneas
    int count = 0;

    while (archivo.available()) {
        buffer[count] = archivo.readStringUntil('\n');
        count++;

        // Cuando tengamos 15 líneas listas, las enviamos en fragmentos de 20 bytes
        if (count == 30) {
            for (int i = 0; i < 30; i++) {
                String registro = buffer[i];
                int len = registro.length();
                int start = 0;

                while (start < len) {
                    String fragmento = registro.substring(start, start + 20);
                    if (deviceConnected && pTxCharacteristic) {
                        pTxCharacteristic->setValue(fragmento.c_str());
                        pTxCharacteristic->notify();
                    }
                    start += 20;
                }
            }
            delay(100);  // Pequeño retraso entre bloques de 15 líneas
            count = 0;  // Reiniciar el contador
        }
    }

    // Enviar las líneas restantes si quedaron menos de 15
    if (count > 0) {
        ...
        delay(100);  // Retraso final si hay registros restantes
    }
