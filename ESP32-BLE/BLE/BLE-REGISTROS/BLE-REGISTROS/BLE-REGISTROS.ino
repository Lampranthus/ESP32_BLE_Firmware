...
const String claveSecreta = "A"; // Define la clave secreta para activar BLE
...
// Callback de BLE para recibir datos (fecha y hora desde el celular)
class MyCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic* pCharacteristic) override {
        String rxValue = pCharacteristic->getValue().c_str();  // Conversión corregida
        if (rxValue == "CLEAR") {
            Serial.println("Comando de borrar recibido. Eliminando registros...");
            
            if (SPIFFS.exists("/log_teclado.txt")) {
                SPIFFS.remove("/log_teclado.txt");  // Borrar el archivo de registros
                Serial.println("Archivo de registros eliminado.");
            }

            // Resetear el índice en la memoria flash
            ...
            if (year > 2000) { // Validar que la fecha recibida es correcta
              ...
              Serial.println("Hora actualizada en el ESP32.");
              horaActualizada = true; // Marcar que ya se recibió la hora
            }else{
              Serial.println("Error al actualizar la hora. Reiniciando...");
              delay(1000);
              ESP.restart();
            }
        }else{
          Serial.println("Error al recir datos. Reiniciando...");
          delay(1000);
          ESP.restart();
        }
    }
...
void setup() {
    ...
    while (!horaActualizada) {
        ...
        // Si pasa el tiempo límite sin recibir la hora, reinicia BLE
        if (millis() - startTime > timeout) {
            Serial.println("Timeout esperando la hora. Reiniciando BLE...");
            BLEDevice::stopAdvertising();  // Detener publicidad BLE
